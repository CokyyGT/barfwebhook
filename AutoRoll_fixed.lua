...
-- ══════════════════════════════════════
--           FIND PLOT & PROMPT
-- Robust: cari plot milik player dengan
-- berbagai path & fallback
-- ══════════════════════════════════════
local function GetPlotsContainer()
    -- Coba berbagai path
    local map = workspace:FindFirstChild("Map")
    if map then
        local p = map:FindFirstChild("Plots")
        if p then return p end
    end
    -- Fallback: langsung di workspace
    local p = workspace:FindFirstChild("Plots")
    if p then return p end
    -- Fallback: cari recursive
    for _, v in ipairs(workspace:GetChildren()) do
        local found = v:FindFirstChild("Plots")
...
local function IsMyPlot(plot)
    -- Cek semua TextLabel di plot, case-insensitive + trim
    local myName = Player.Name:lower():gsub("^%s*(.-)%s*$", "%1")
    for _, desc in ipairs(plot:GetDescendants()) do
        if desc:IsA("TextLabel") then
            local t = desc.Text:lower():gsub("^%s*(.-)%s*$", "%1")
            if t == myName then return true end
        end
        -- Cek juga StringValue (beberapa game pakai ini)
        if desc:IsA("StringValue") and desc.Name:lower():find("owner") then
            local t = desc.Value:lower():gsub("^%s*(.-)%s*$", "%1")
            if t == myName then return true end
        end
    end
    return false
...
local function FindPromptInPlot(plot)
    -- Coba path utama: RollPlatform > Lever > LevelPrompt > RollSeeds
    ...
    if rp then
        ...
        if lv then
            ...
            if lp then
                local p = lp:FindFirstChild("RollSeeds")
                if p then return p end
            end
            -- Fallback: semua ProximityPrompt di Lever
            for _, d in ipairs(lv:GetDescendants()) do
                if d:IsA("ProximityPrompt") then return d end
            end
        end
        -- Fallback: semua ProximityPrompt di RollPlatform
        for _, d in ipairs(rp:GetDescendants()) do
            if d:IsA("ProximityPrompt") then return d end
        end
    end
    -- Fallback: cari semua ProximityPrompt di plot
    -- prioritaskan yang namanya Roll/Seed
    local fallback = nil
    for _, d in ipairs(plot:GetDescendants()) do
        if d:IsA("ProximityPrompt") then
            if d.Name:lower():find("roll") or d.Name:lower():find("seed") then
                return d
            end
...
local function FindPlotAndPrompt()
    ...
    if not plotsContainer then
        -- Last resort: tunggu Map load dulu max 5s
        local t = 0
        while t < 5 and not plotsContainer do
    ...
    for _, plot in ipairs(plotsContainer:GetChildren()) do
        if not plot.Name:match("^Plot") then
            -- no continue keyword untuk kompatibilitas executor
        else
            if IsMyPlot(plot) then
                myPlot     = plot
                rollPrompt = FindPromptInPlot(plot)
                if rollPrompt then return true end
                -- Plot ketemu tapi prompt belum ada, tunggu sebentar
                task.wait(1)
                rollPrompt = FindPromptInPlot(plot)
                if rollPrompt then return true end
            end
...
local UI_W        = isMobile and math.floor(screenSize.X * 0.88) or 380
local MINI_H      = 44
local SECTION_PAD = isMobile and 6 or 8
...
main.Position          = isMobile and UDim2.new(0.5, -math.floor(UI_W/2), 0.5, -250) or UDim2.new(0, 20, 0.5, -250)
main.Parent            = sg
corner(main, 12)
stroke(main, C.border, 1)

-- UIScale untuk resize UI
...
contentList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    ...
    local maxH = isMobile and math.floor(screenSize.Y * 0.85) or 650
    main.Size = UDim2.new(0, UI_W, 0, math.min(h, maxH))
end)
...
local cellW = isMobile and math.floor((UI_W - 40) / 3) or 100
rarityGrid.CellSize             = UDim2.new(0, cellW, 0, isMobile and 26 or 28)
rarityGrid.CellPadding          = UDim2.new(0, isMobile and 4 or 5, 0, isMobile and 4 or 5)
...
task.defer(function()
    ...
    local cellH = isMobile and 26 or 28
    local cellGap = isMobile and 4 or 5
    rarityCard.Size = UDim2.new(1, 0, 0, rows * cellH + (rows-1)*cellGap + 16)
end)
...
minBtn.MouseButton1Click:Connect(function()
    ...
    if minimized then
        ...
        local h = contentList.AbsoluteContentSize.Y + SECTION_PAD*2 + 44
        local maxH2 = isMobile and math.floor(screenSize.Y * 0.85) or 650
        main.Size = UDim2.new(0, UI_W, 0, math.min(h, maxH2))
    end
