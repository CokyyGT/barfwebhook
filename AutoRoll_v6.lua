...
local function RunLoop()
    ...
    while Running do
        ...
        -- Tunggu workspace stabil (tunggu seed muncul dulu, baru monitor)
        WaitWorkspaceStable()

        -- Scan seed sekarang workspace sudah stabil
        local seeds = ScanSeeds()
        ...
        local bonusName = FindBonusObject()
        if bonusName then
            -- cek dulu apakah bertahan lebih dari 0.8 detik
            task.wait(0.8)
            bonusName = FindBonusObject()
            if bonusName then
                -- beneran aktif, bukan flash sesaat
                addLog("🍀 LUCK TERDETECT: " .. bonusName .. "!", C.gold)
                -- kasih delay dulu biar seed sempat muncul & kebeli
                task.wait(1.5)
                -- scan & beli seed yang muncul
                local bonusSeeds = ScanSeeds()
                for _, seed in ipairs(bonusSeeds) do
                    if TARGET_RARITIES[seed.rarity] then
                        addLog("🌟 " .. seed.name .. " [" .. seed.rarity .. "]!", C.gold)
                        local buyOk = false
                        for _fire = 1, 5 do
                ...
                -- tunggu sampai bonus selesai
                local bonusWait = 0
                while FindBonusObject() and bonusWait < 60 do
                    local current = FindBonusObject()
                    if current and current ~= bonusName then
                        addLog("⬆ Naik ke " .. current .. "!", C.gold)
                        bonusName = current
                        -- scan lagi kalau naik tier, seed baru mungkin muncul
                        task.wait(1.5)
                        local tierSeeds = ScanSeeds()
                        for _, seed in ipairs(tierSeeds) do
                            if TARGET_RARITIES[seed.rarity] then
                                addLog("🌟 " .. seed.name .. " [" .. seed.rarity .. "]!", C.gold)
                                local buyOk = false
                                for _fire = 1, 5 do
                ...
                addLog("✅ Luck selesai, lanjut roll!", C.green)
            end
            -- kalau < 0.8 detik = flash sesaat, abaikan
        end

        if gotTarget then
            -- Ada yang dibeli, kasih jeda sebelum roll berikutnya
            task.wait(ROLL_DELAY)
        else
            -- Ga ada target, langsung roll secepat mungkin
