...
local function LoadConfig()
    ...
    if data.webhook_url     and data.webhook_url ~= ""   then WEBHOOK_URL     = data.webhook_url     end
    if data.webhook_enabled ~= nil                        then WEBHOOK_ENABLED = data.webhook_enabled end
end

-- ══════════════════════════════════════
--   WEBHOOK CONFIG (harus sebelum LoadConfig!)
-- ══════════════════════════════════════
local WEBHOOK_URL     = ""
...
local function loadRunState()
    local ok, raw = pcall(function() return readfile("autoroll_run_state.json") end)
    if ok and raw then
        local ok2, data = pcall(function() return HttpService:JSONDecode(raw) end)
        if ok2 and data then return data end
    end
    return {running=false, totalRolls=0, totalBought=0}
end

local runState = loadRunState()
local Running     = runState.running
local TotalRolls  = runState.totalRolls
local TotalBought = runState.totalBought
local StartTime   = os.time()

-- Save run state setiap detik
local function saveRunState()
    pcall(function() 
        writefile("autoroll_run_state.json", HttpService:JSONEncode({
            running = Running,
...
local function GetRarityEmoji(rarity)
    ...
    if r == "common"      then return "⬜"
    elseif r == "uncommon"   then return "🟩"
    elseif r == "rare"       then return "🟦"
    elseif r == "epic"       then return "🟪"
    elseif r == "legendary"  then return "🟨"
    elseif r == "secret"     then return "🔴"
    elseif r == "prismatic"  then return "🌈"
    elseif r == "divine"     then return "💜"
    elseif r == "exotic"     then return "🟠"
    elseif r == "transcended" then return "✨"
    elseif r == "celestial"  then return "🌟"
    elseif r == "eternal"    then return "👑"
    else return "❓" end
end
...
resetWrap.Size                   = UDim2.new(1,-SECTION_PAD*2,0,26)
resetWrap.LayoutOrder            = sectionOrder
resetWrap.Parent                 = scroll

local resetBtn = mkBtn(resetWrap, {
    text  = "🔄  Reset Stats",
    bg    = C.surface,
    color = C.muted,
    size  = 10,
...
resetBtn.MouseButton1Click:Connect(function()
    ...
    saveRunState()
    resetBtn.Text = "✅  Reset!"
    resetBtn.BackgroundColor3 = C.green
...
seedNameGrid.CellSize            = UDim2.new(0, 158, 0, 26)
seedNameGrid.CellPadding         = UDim2.new(0, 4, 0, 4)
...
UpdateWhStatus()
-- Sync urlBox dengan URL yang sudah di-load dari config
urlBox.Text = WEBHOOK_URL
...
-- Auto-resume jika di execute ulang saat Running
if Running and not _G.AutoRollResumed then
    _G.AutoRollResumed = true
