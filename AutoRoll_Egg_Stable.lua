...
-- Mode tabs
local ActiveMode = "Roll"
local modeLabel = mkLabel(header, {text="🎰 Roll", font=Enum.Font.GothamBold, size=10, color=C.white, sz=UDim2.new(0.3,0,1,0), pos=UDim2.new(0.5,0,0,0), z=21})

header.MouseButton1Click:Connect(function()
    -- Quick toggle Roll/Egg dengan double click
end)
...
local function FindPetMerchant()
    petMerchant = workspace:FindFirstChild("PetMerchant") or workspace:FindFirstChild("petmerchant") or workspace:FindFirstChild("Pet Merchant")
    ...
    for i = 1, 5 do
        ...
        if stock and lever then
            ...
            for _, d in ipairs(stock:GetDescendants()) do
                if d:IsA("TextLabel") and d.Text ~= "" then eggName = d.Text; break end
            end
            local pp = lever:FindFirstChildWhichIsA("ProximityPrompt", true)
            if eggName and pp then
                table.insert(results, {podium=i, eggName=eggName, prompt=pp})
            end
        end
    end
    return results
